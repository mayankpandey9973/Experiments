...
of data) as judged by cifar100_eval.py.

Speed:
On a single Tesla K40, cifar100_train.py processes a single batch of 128 images
...
tf.app.flags.DEFINE_string('eval_dir', '/home/mayankp/tfRuns/outputs/normalized_relu/tmp/eval-' + cifar100.name + '_stoch',
                           """Directory where to write event logs.""")
tf.app.flags.DEFINE_string('eval_data', 'test',
                           """Either 'test' or 'train_eval'.""")
tf.app.flags.DEFINE_string('checkpoint_dir', '/home/mayankp/tfRuns/outputs/normalized_relu/tmp/trainL2-' + cifar100.name,
...
def eval_once(saver, summary_writer, top_k_op, summary_op, inputs, logitsArr, labels, logitsAct):
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    # Batch generator
    batcher = cifar100.Cifar10BatchGenerator(
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    inputs = cifar100.ram_inputs(unit_variance=True, is_train=False)
