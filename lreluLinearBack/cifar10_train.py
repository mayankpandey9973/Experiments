...
def train():
  ...
  with tf.Graph().as_default():
    ...
    while not batcher.is_done():
      step += 1
      num10K = step / 10000
      curReluDecay = 1. - 0.99**(math.floor(num10K))
      ...
      if step % 10 == 0:
        ...
        format_str = ('%s: step %d, loss = %.4f (%.4f examples/sec; %.4f '
                      'sec/batch), relu decay now = %.5f, writing to ' + FLAGS.train_dir + '.')
        print (format_str % (datetime.now(), step, loss_value,
                             examples_per_sec, sec_per_batch, curReluDecay))
