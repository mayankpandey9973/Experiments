...
def mreluDecayFunc(x, scale):
    return tf.exp(x * scale)

def modifiedRelu_train(x, scale):
    noise_shape = array_ops.shape(x)
    pos = tf.nn.relu(x)
    neg = x - pos
    theta = tf.floor(tf.minimum(tf.ones(noise_shape), mreluDecayFunc(x, scale)) + tf.random_uniform(noise_shape, maxval = 1.0))
...
#tf.scalar_summary('reluVal', modifiedRelu(-1.0))
  ...
  with tf.control_dependencies([loss_averages_op]):
    ...
    gradsConv0 = opt2.compute_gradients(total_loss, var_list = tf.get_collection('relu_scales', 'conv0'))
